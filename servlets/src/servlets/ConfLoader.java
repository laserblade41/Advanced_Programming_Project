...
/**
 * Application servlet that handles the {@code /upload} endpoint for configuration deployment.
 *
 * <p>This is a reference servlet bundled with the computational-graph dashboard. Library
 * consumers may use it as-is or as a pattern for building their own upload handlers.</p>
 *
 * <p><strong>GET:</strong> Builds a {@link configs.Graph} from the current topics in
 * {@link graph.TopicManagerSingleton} and returns an HTML visualization generated by
 * {@link views.HtmlGraphWriter}.</p>
 *
 * <p><strong>POST:</strong> Accepts a configuration file via multipart form data
 * ({@code filename} and {@code file} fields) or, as a fallback, plain request body and
 * query parameters. The file is saved under {@code config_files/}, loaded through
 * {@link configs.GenericConfig}, and the resulting graph HTML is returned. On error, a
 * {@code 400 Bad Request} plain-text response is sent.</p>
 *
 * <p><strong>Thread safety:</strong> This servlet is not thread-safe for concurrent POST
 * requests. It shares {@link graph.TopicManagerSingleton} state and performs filesystem
 * writes without synchronization; concurrent uploads may race.</p>
 *
 * @see Servlet
...
public class ConfLoader implements Servlet {
    ...
    /**
     * Processes a GET or POST request to the {@code /upload} endpoint.
     *
     * <p>For POST requests, parses multipart or plain body content, saves the configuration
     * file, loads agents via {@link configs.GenericConfig}, rebuilds the graph, and writes
     * an HTML response. For GET requests, renders the current graph without uploading.
     * Errors are reported as {@code 400 Bad Request}.</p>
     *
     * @param ri the parsed request, including HTTP method, parameters, and body content
